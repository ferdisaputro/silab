...
<div class="flex items-center">
    <input id="{{ $name }}" value="{{ $value }}" name="{{ $name }}" type="checkbox" {{ $checked? "selected" : "" }} {{ $attributes->merge(['class' => 'w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600']) }}>
    <label for="{{ $name }}" class="text-sm font-medium text-gray-900 ms-2 dark:text-gray-300">{{ $label }}</label>
</div>
