    ...
    >
        <div class="relative group select-form-container">
            ...
            >
                ...
                {{-- Default text value --}}
                <span wire:ignore x-text="(selectedItem && selectedItem.value !== '')? selectedItem.text : '{{ $label }}'" class="absolute mt-2 border-0 pointer-events-none left-4 select-form-text w-fit h-fit"></span>
            ...
            <div x-show="isOpen" wire:ignore x-data="selectAdvance()" x-init="convertOption($el)" class="fixed z-30 p-1 mt-2 space-y-1 bg-white rounded-md shadow-lg select-form-menu ring-1 ring-black ring-opacity-5">
                <!-- Search input -->
                ...
                <div class="overflow-x-auto max-h-60">
                    ...
                    <template x-for="(item, index) in options" :key="index">
                        <span x-show="search === '' || item.value.toLowerCase().includes(search.toLowerCase()) || item.text.toLowerCase().includes(search.toLowerCase())" :data-value="item.value" class="block px-4 py-2 text-gray-700 rounded-md cursor-pointer select-form-item hover:bg-gray-100 active:bg-blue-100"
...
@pushOnce('scripts')
    @script
        <script>
            Alpine.data('selectAdvance', () => {
            return {
                options: null,
                convertOption(el) {
                    const currentParentElement = el;
                    
                    this.options = [{value: "", text: "{{ $label }}"}]
                        .concat(Array.from(currentParentElement.querySelectorAll('option')).map(option => {
                            return {value: option.value, text: option.textContent, selected: option.selected}
                        }));

                    currentParentElement.querySelectorAll('option').forEach(element => {
                        element.remove()
                    });
                }
            }
