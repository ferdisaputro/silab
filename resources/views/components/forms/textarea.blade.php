...
<div {{ $attributes->merge(['class']) }}>
   <div class="relative">
      <textarea {{ $attributes->whereStartsWith('wire') }} id="{{ $name }}" name="{{ $name }}" value="{{ $value }}" placeholder="{{ $placeholder }}" {{ $disabled? "disabled" : '' }}
         class="{{ $class }} block px-4 pb-2.5 pt-4 w-full text-sm bg-transparent disabled:bg-primaryGrey rounded-lg border-1 border-gray-200 appearance-none dark:border-gray-600 dark:focus:border-primaryLightTeal focus:outline-none focus:ring-1 focus:ring-primaryTeal focus:border-primaryTeal peer
