<x-container x-data="Object.assign(selectSubject(), selectLecturer())">
    <div>
        <x-modals.modal identifier="selectSubjectState" max_width="max-w-3xl">
            <livewire:pages.subject-instructor.subject-table />
        </x-modals.modal>

        <x-modals.modal identifier="selectLecturerState" max_width="max-w-3xl">
            <livewire:pages.subject-instructor.lecturer-table />
        </x-modals.modal>
    </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div class="flex flex-col gap-4 mt-3 md:col-span-2 md:flex-row">
                    <div class="flex flex-col justify-end flex-1 md:col-span-2">
                        ...
                        <x-forms.input value="{{ $selectedSubject }}" name="matkul" label="Matakuliah" readonly="true" height="h-14">
                            <x-buttons.outline x-on:click="selectSubjectState = true" color="purple" height="" class="absolute w-fit right-1.5 top-1.5 bottom-1.5">Pilih</x-buttons.outline>
                        </x-forms.input>
                    </div>
                    <div class="flex flex-col justify-end flex-1 md:col-span-2">
                        <h5 class="mb-2 font-semibold ms-2">
...
@pushOnce('scripts')
    @script
        <script>
            Alpine.data('selectSubject', () => {
                return {
                    selectSubjectState: false,
                    showSelectSubject (id) {
                        // $wire.dispatch('initSelectSubject', {id: id});
                        this.selectSubjectState = true;
                    }
                }
            })
